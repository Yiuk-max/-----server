...
void Sha256::update(const uint8_t* data, std::size_t len) {
    ...
    while (len > 0) {
        if (buffer_len_ == 0 && len >= 64) {
            ...
            std::memcpy(buffer_ + buffer_len_, data + idx, take);
            buffer_len_ += take;
            idx += take;
            len -= take;
            if (buffer_len_ == 64) {
...
std::string Sha256::final_hex() {
    ...
    const std::size_t zero_len = (buffer_len_ < 56) ? (55 - buffer_len_) : (119 - buffer_len_);
    const std::size_t pad_len = 1 + zero_len + 8;

    pad[0] = 0x80;
    for (int i = 0; i < 8; ++i) {
        pad[pad_len - 8 + i] = static_cast<uint8_t>((bit_len >> (56 - 8 * i)) & 0xFF);
    }
    update(pad, pad_len);
