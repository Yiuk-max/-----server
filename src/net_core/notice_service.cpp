...
void NoticeService::send_to_user_with_id(int uid, const ChatMessageData& data) {
    auto target = session_manager::get_instance().find_session(uid);
    if (target) {
        target->package_chat_message(data);
    }
}

void NoticeService::send_to_users_with_id(const std::vector<int>& uid_list, const ChatMessageData& data) {
    // 广播：Envelope 构造 + 序列化各做一次，再批量查在线会话逐个发送，
    // 避免 N 次 protobuf 序列化与 N 次 find_session 加锁。
    chat_proto::Envelope env;
    build_chat_envelope(env, data);
    std::string payload;
    if (!env.SerializeToString(&payload)) {
        return;
    }
