...
void social_module::add_friend(int friend_UID){
    std::string target_name = repo_hub_->accounts()->load_account(friend_UID)->getName();
...
void social_module::send_friend_request(int receiver_UID, std::string &apply_message){
    ...
    auto receiver = repo_hub_->accounts()->load_account(receiver_UID);//效验被添加者是否存在
    if(receiver){
        std::string message;
...
void social_module::handle_friend_request(int sender_UID, bool accept){
    std::string notify;
    auto self_account = repo_hub_->accounts()->load_account(user_UID_);
    if (!self_account) {
        return;
    }
