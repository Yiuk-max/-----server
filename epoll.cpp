...
void sub_reactor::add_connect(int new_client_fd)
{
    ...
    auto manager = std::make_shared<Text_msg_handler>(new_client_fd, epoller_fd_);
    {
...
void sub_reactor::pool_add_task(std::string received_data, int fd)
{
    ...
    pool->add_task([pool, fd, received_data]()
                   {
                        std::shared_ptr<Text_msg_handler> manager;
                        {
                            std::lock_guard<std::mutex> lock(client_mutex);
        ...
        for (int i = 0; i < num_events; ++i) {
            ...
            if (fd == server_fd) {
                ...
                if (client_fd != -1) {
                    ...
                    auto manager = std::make_shared<Text_msg_handler>(client_fd);
                    {
                        std::lock_guard<std::mutex> lock(client_mutex);
